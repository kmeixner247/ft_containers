...
/*   Updated: 2022/10/04 01:12:43 by kmeixner         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
public:
	...
	constant_RBT_iterator(RBT_iterator<T, Tree> const &rhs) {*this = rhs;}
	...
	constant_RBT_iterator &operator++()
	{
		...
		if (!this->_currentnode->parent)
			this->_currentnode = ConstNode<T>(this->_tree->cend());
		this->_current = this->_currentnode->content;
		return (*this);
	}
	...
	constant_RBT_iterator &operator--()
	{
		if (this->_currentnode->lc)
		{
			this->_currentnode = this->_current->lc;
		...
		if (!this->_currentnode->parent)
			this->_currentnode = this->_tree->getEnd();
		this->_current = this->_currentnode->content;
		return (*this);
	}
